...
class Tracker:
    ...
    def draw_annotations(self, video_frames, tracks, team_ball_control):
        ...
        player_names_list = ["Alex","Jordan","Taylor","Morgan","Casey","Riley","Jamie","Cameron","Drew","Quinn"]
        player_id_to_name = {}
        all_player_ids = set()
        ...
        previous_ratings = {player_id: round(random.uniform(5.0, 10.0),1) for player_id in all_player_ids}
        rating_update_interval = 120  # ~4 sec at 30fps
        ...
        for frame_num, frame in enumerate(video_frames):
            ...
            if frame_num % rating_update_interval == 0:
                for player_id in previous_ratings.keys():
                    previous_ratings[player_id] = round(random.uniform(5.0, 10.0),1)

            for track_id, player in player_dict.items():
                color = player.get("team_color", (0,0,255))
                frame = self.draw_ellipse(frame, player["bbox"], color, track_id)

                player['name'] = player_id_to_name[track_id]
                player['rating'] = previous_ratings[track_id]

                x_center, _ = get_center_of_bbox(player["bbox"])
                y_bottom = int(player["bbox"][3])
                text = f"{player['name']} ({player['rating']})"
                cv2.putText(frame, text, (x_center-30, y_bottom+90),  # moved down 50 pixels
